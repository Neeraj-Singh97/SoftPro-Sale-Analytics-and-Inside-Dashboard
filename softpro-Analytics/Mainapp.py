...
#C:\Users\asus\OneDrive\Desktop\NeerajSingh-Ai-Ml\softpro-Analytic\ffmpeg\bin
#os.environ["PATH"] += os.pathsep + r"D:\Training\spi\Python-with-Datascience\my-softpro-project\softpro-Analytics\ffmpeg\bin"
#os.environ["path"]+= os.pathsep + r"C:Users\asus\OneDrive\Desktop\NeerajSingh-Ai-Ml\softpro-Analytic\ffmpeg\bin"
...
if merged is not None and len(merged) > 0:
    ...
    colA, colB, colC ,colE = st.columns(4)
    with colA:
    ...
    with colE:
        if "year" in merged.columns:
            fig5 = px.bar(merged.fillna({"year": "Unknown"}), x="year", color="sentiment", title="Sentiment by Year")
    ...
    def gen_recos(df_: pd.DataFrame):
        ...
        return recos
        if any(k in text_all for k in ["demo", "project", "portfolio"]):
            recos.append("Practical exposure → highlight live projects, capstones, and portfolio-building opportunities.")
        return recos
        if any(k in text_all for k in ["review", "testimonials", "alumni"]):
            recos.append("Social proof → showcase reviews, testimonials, and alumni success stories.")
        return recos                                                                                                                                                                                                                                                                                                                                                                                                                                              
